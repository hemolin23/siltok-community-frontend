import { Boxes, FileStack, Gauge, MessageCircle, MessageSquareWarning, Plus, Users } from 'lucide-react';
...
export function WorkspaceFrame({ active, eyebrow, title, description, createHref, createLabel, children }: {
  ...
  return <main className="community-shell">
    ...
    <header className="canvas-header community-header"><Brand /><nav><a href="/">首页</a><a className={active === 'projects' ? 'active' : ''} href="/projects">共创项目</a><a className={active === 'workflows' ? 'active' : ''} href="/workflows">工作流</a><a className={active === 'issues' ? 'active' : ''} href="/issues">问题反馈</a><a href="/#benchmarks">模型实测</a></nav><div className="header-actions"><a className="quiet-action" href="/login"><MessageCircle /> 微信登录</a><a className="neon-action" href={createHref}><Plus /> {createLabel}</a></div></header>
