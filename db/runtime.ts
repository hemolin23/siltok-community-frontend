  ...
  await db.batch([
    ...
    db.prepare(`INSERT OR IGNORE INTO templates
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'public', ?, ?, ?)`).bind(
        ...
        JSON.stringify(['角色卡', '分镜脚本', '参考图']), 'verified_run', 'official', 'beta', 0, now - 3600000, now),
    db.prepare(`INSERT OR IGNORE INTO templates
      (id, owner_id, kind, slug, title, category, summary, model_name, hardware_sku, instructions, input_schema_json, evidence_level, source_type, status, visibility, install_count, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'public', ?, ?, ?)`).bind(
        ...
        JSON.stringify(['商品图', '卖点', '品牌规范']), 'community_testing', 'official', 'beta', 0, now - 3500000, now),
    db.prepare(`INSERT OR IGNORE INTO templates
      (id, owner_id, kind, slug, title, category, summary, model_name, hardware_sku, instructions, input_schema_json, evidence_level, source_type, status, visibility, install_count, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'public', ?, ?, ?)`).bind(
        ...
        JSON.stringify(['错误日志', '工作流 JSON', '失败样片']), 'official_reviewed', 'official', 'ready', 0, now - 3400000, now),
    db.prepare(`INSERT OR IGNORE INTO templates
      (id, owner_id, kind, slug, title, category, summary, model_name, hardware_sku, instructions, input_schema_json, evidence_level, source_type, status, visibility, install_count, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'public', ?, ?, ?)`).bind(
        ...
        JSON.stringify(['角色卡', '分镜表', '参考图']), 'verified_run', 'official', 'ready', 0, now - 3300000, now),
    db.prepare(`INSERT OR IGNORE INTO templates
      (id, owner_id, kind, slug, title, category, summary, model_name, hardware_sku, instructions, input_schema_json, evidence_level, source_type, status, visibility, install_count, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'public', ?, ?, ?)`).bind(
        ...
        JSON.stringify(['商品图', '卖点表', '参考视频']), 'community_testing', 'official', 'beta', 0, now - 3200000, now),
    db.prepare(`INSERT OR IGNORE INTO templates
      (id, owner_id, kind, slug, title, category, summary, model_name, hardware_sku, instructions, input_schema_json, evidence_level, source_type, status, visibility, install_count, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'public', ?, ?, ?)`).bind(
        ...
        JSON.stringify(['剧本', '分镜描述', '画幅']), 'lab_testing', 'official', 'testing', 0, now - 3100000, now),
  ]);
